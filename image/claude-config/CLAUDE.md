...
- Build: `cd rust && k3s-claude cargo-lock build --release 2>&1`
- Check: `cd rust && k3s-claude cargo-lock check 2>&1`
- Clippy: `cd rust && k3s-claude cargo-lock clippy --release -- -D warnings 2>&1`
- Fmt: `cd rust && k3s-claude cargo-lock fmt 2>&1`

## Rules

- Use `k3s-claude cargo-lock` for all cargo commands (build, check, clippy, fmt) to serialize builds across pods.
